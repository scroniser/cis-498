        ...
        <table id="responsive-table" class="display responsive no-wrap" cellspacing="0" width="100%">
            ...
            <c:forEach var="room" items="${rooms}">
                <tr>
                    ...
                    <% if (session.getAttribute("usertype").toString().equalsIgnoreCase("A")) {%>
                    <td><a href="ScheduleController?action=selectProf&callNumber=<c:out value="${requestScope.callNumber}"/>&selectedRoomNum=<c:out value="${room.room}"/>&selectedBuilding=<c:out value="${room.department}"/>">Select</a></td>
                    <% } %>
                </tr>
            </c:forEach>
        </tbody>
