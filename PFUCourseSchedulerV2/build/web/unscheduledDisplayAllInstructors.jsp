        ...
        <table id="responsive-table" class="display responsive no-wrap" cellspacing="0" width="100%">
            ...
            <c:forEach var="instructor" items="${instructors}">
                <tr>
                    ...
                    <% if (session.getAttribute("usertype").toString().equalsIgnoreCase("A")) {%>
                    <td><a href="UnscheduledCoursesController?action=reschedule&callNumber=<c:out value="${requestScope.callNumber}"/>&selectedRoomNum=<c:out value="${requestScope.selectedRoomNum}"/>&selectedBuilding=<c:out value="${requestScope.selectedBuilding}"/>&selectedInstructor=<c:out value="${instructor.instructor}"/>">Select</a></td>
                    <% } %>
                </tr>
            </c:forEach>
        </tbody>
